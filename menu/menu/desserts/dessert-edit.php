...
if (empty($id)) {
  header('Location: dessert-menu.php');
  exit; # 結束 php 程式, die()
}
        ...
        <div class="card">
          <div class="card-body">
            ...
            <form name="form1" onsubmit="sendData(event)">
              ...
              <div class="mb-3">
                <label for="name" class="form-label"><span class="required">**</span> 甜點名稱</label>
                <input type="text" class="form-control" id="name" name="name" value="<?= $row['name'] ?>">
                <div class="form-text"></div>
              </div>

              <div class="mb-3">
                <label for="price" class="form-label"><span class="required">**</span> 甜點售價</label>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <a href="dessert-menu.php" class="btn btn-primary">到列表頁</a>
      </div>
    </div>
  </div>
</div>
...
<script>
  ...
  const sendData = e => {
    ...
    if (nameField.value.length < 2) {
      ...
      nameField.nextElementSibling.innerHTML = '請輸入甜點名稱';
    }

    if (priceField.value.length < 2) {
      isPass = false;
      priceField.style.border = '1px solid red';
      priceField.nextElementSibling.innerHTML = '請輸入甜點售價';
    }
